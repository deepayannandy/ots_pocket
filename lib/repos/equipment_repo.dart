...
class EquipmentRepository {
  ...
  EquipmentRepository({this.restClient});

  Future<String> addEquipment(
      {required equipmentsDetails EquipDetails}) async {
  ...
  Future<List<equipmentsDetails>> getEquipments() async {
    String endpoint = APIEndpoint.ENDPOINT_GET_ALL_EQUIPMENTS;

    Response response = await restClient?.get(
      endpoint: endpoint,
    );
    if (response.statusCode == 200 || response.statusCode == 201) {
      List<equipmentsDetails> allEquipments = json
          .decode(response.body)
          .map<equipmentsDetails>(
              (json) => equipmentsDetails.fromJson(json))
          .toList();
  ...
  //by deep
  Future<equipmentsDetails> getSpecificEquipments(
  ...
  // patch consumable
  Future<String> patchEquipment(
      {required equipmentsDetails equipment}) async {
    String endpoint =
